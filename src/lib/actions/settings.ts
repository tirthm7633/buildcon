...
import { canManageRoleAndStatus, isOwner } from "@/lib/auth";
...
export async function inviteStaffMember(values: InviteStaffFormValues) {
  if (!(await canManageRoleAndStatus())) return { error: "Only the owner or a manager can invite staff." };
...
export async function updateStaffRole(id: string, role: UserRole) {
  if (!(await canManageRoleAndStatus())) return { error: "Only the owner or a manager can change roles." };
...
export async function setStaffActive(id: string, isActive: boolean) {
  if (!(await canManageRoleAndStatus())) return { error: "Only the owner or a manager can update staff." };
