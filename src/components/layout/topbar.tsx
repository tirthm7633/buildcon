...
export function Topbar({
  ...
  return (
    <header className="flex h-16 shrink-0 items-center gap-3 border-b border-border bg-card px-4 lg:hidden">
      ...
      <Logo logoUrl={logoUrl} variant="dark" showWordmark={false} size={44} className="flex-1" />

      <Sheet open={open} onOpenChange={setOpen}>
        <SheetContent side="left" className="w-72 bg-sidebar p-0">
          <SheetHeader className="h-20 flex-row items-center border-b border-sidebar-border px-5">
            <SheetTitle>
              <Logo logoUrl={logoUrl} variant="light" showWordmark={false} size={56} />
            </SheetTitle>
          </SheetHeader>
          <div className="flex h-[calc(100%-5rem)] flex-col gap-3 overflow-y-auto px-3 py-3">
